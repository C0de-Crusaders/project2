<script>

</script>



<div class="hero">
  <div class="hero-content">
    <h1>Welcome to PlanNext</h1>
    <p>Manage Your Events With Ease</p>
    <p>Plan your events with ease and organization.</p>
    <button class="get-started" on:click={() => window.location.href = 'signup'}>Get Started</button>
  </div>
</div>

<section class="features">
  <h2>Our Features</h2>
  <p>Explore our diverse set of features for effective event management.</p>
  <div class="feature-cards">
    <div class="card">
      <h3>EVENTS</h3>
      <p>Manage Events</p>
      <div class="card-footer">
        <strong>Event Calendar</strong>
        <p>Add, edit, and view events</p>
      </div>
    </div>
    <div class="card">
      <h3>REMINDERS</h3>
      <p>Event Reminders</p>
      <div class="card-footer">
        <strong>Reminders</strong>
        <p>Stay updated with event notifications</p>
      </div>
    </div>
    <div class="card">
      <h3>IMAGES</h3>
      <p>Upload Images</p>
      <div class="card-footer">
        <strong>Add Images</strong>
        <p>Manage Events with Image</p>
      </div>
    </div>
  </div>
</section>

<footer>
  <p>© 2024 PlanNext. All Rights Reserved.</p>
</footer>

<style>
  .hero {
    background-color: #4a7c59;
    color: white;
    padding: 2rem;
    text-align: center;
  }

  .hero-content {
    max-width: 800px;
    margin: 0 auto;
  }

  .get-started {
    background-color: #2c5f2d;
    color: white;
    padding: 0.5rem 1rem;
    border: none;
    cursor: pointer;
  }

  .features {
    text-align: center;
    padding: 2rem;
  }

  .feature-cards {
    display: flex;
    justify-content: center;
    gap: 1rem;
  }

  .card {
    background-color: #a3c9a8;
    padding: 1rem;
    border-radius: 8px;
    width: 200px;
    text-align: center;
  }

  .card-footer {
    background-color: #4a7c59;
    color: white;
    padding: 0.5rem;
    border-radius: 0 0 8px 8px;
  }

  footer {
    background-color: #a3c9a8;
    text-align: center;
    padding: 1rem;
  }
</style>